...
class HumanLikeDecisionMakingUnderUncertaintySystem:
    ...
    def make_decision(self, observations, available_sentences):
        ...
        # Calculate probability/evidence for toxity
        weightning_table["eat"] = evidence_for_non_toxic if evidence_for_non_toxic > 0 else self.epsilon
        weightning_table["explorer"] = evidence_for_toxic if evidence_for_toxic > 0 else self.epsilon

        for key in solution_table.keys():
            utility = math.pow(utility_table[key], self.alpha)
