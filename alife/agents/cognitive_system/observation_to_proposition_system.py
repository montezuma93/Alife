...
class MultiplePropositionSystem(OberservationToPropositionSystem):
    ...
    def observation_to_proposition(self, color_proposition: ColorProposition, observated_objects: list, reward: Reward):
        ...
        generated_propositions.append(([color_proposition], Implication(reward, evidence)))
        combinations = []
        for i in range(1, len(observated_objects)+1):
            ...
            for entry in next_list:
                ...
                generated_propositions.append((entry, Implication(reward, evidence)))
...
class OccamsRazorMultiplePropositionSystem(OberservationToPropositionSystem):
    ...
    def observation_to_proposition(self, color_proposition: ColorProposition, observated_objects: list, reward: Reward):
        ...
        print(evidence_per_combination_part)
        generated_propositions = []
        generated_propositions.append(([color_proposition], Implication(reward, evidence_per_combination_part*(max_length_of_proposition.get(len(observated_objects))))))
        for i in range(1, len(observated_objects)+1):
            next_list = [list(x) for x in itertools.combinations(observated_objects, i)]
            for entry in next_list:
                entry.append(color_proposition)
                print(entry)
                print(evidence_per_combination_part*len(entry))
                generated_propositions.append((entry, Implication(reward, evidence_per_combination_part*(max_length_of_proposition.get(len(observated_objects))-len(entry)+1))))
...
class SinglePropositionSystem(OberservationToPropositionSystem):
    ...
    def observation_to_proposition(self, color_proposition: ColorProposition, observated_objects: list, reward: Reward):
        ...
        return [(observated_objects, Implication(reward, evidence))]
